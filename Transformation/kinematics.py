...
def eulerrmx(euler_angle, euler_vector):
    ...
    euler_vector : 3 element radian array
...
def rmxeuler(rot_mat):
    ...
    Return  : euler angle & euler vector in radian anlge
    """
...
def rmxexyz(rot_mat):
    """
    Compute the Euler angles of a X-Y-Z rotation, given the attitude matrix.
    ...
    euler_angles : np.array
        Euler angles from a X-Y-Z rotation, in radians. Array of 3 elements.
    """
    a11, a12, a21, a22, a31, a32, a33 = rot_mat[0, 0], rot_mat[0, 1], rot_mat[1, 0], rot_mat[1, 1], rot_mat[2, 0], rot_mat[2, 1], rot_mat[2, 2]
...
def rmxezxy(rot_mat):
    """
    Compute the Euler angles of a Z-X-Y rotation, given the rotation matrix.
...
def rmxezxz(rot_mat):
    """
    Compute the Euler angles of a Z-X-Z rotation, given the rotation matrix.
    ...
    """
    a11, a12, a13, a23, a31, a32, a33 = rot_mat[0, 0], rot_mat[0, 1], rot_mat[0, 2], rot_mat[1, 2], rot_mat[2, 0], rot_mat[2, 1], rot_mat[2, 2]
...
def rmxezyx(rot_mat):
    """
    Compute the Euler angles of a Z-Y-X rotation, given the rotation matrix.
...
def quatrmx(quaternion):
    ...
    q23, q24, q34 = 2*quaternion[1]*quaternion[2], 2*quaternion[1]*quaternion[3], 2*quaternion[2]*quaternion[3]
...
def exyzrmx(euler_angles):
    rot_mat=rotmax(euler_angles[2]*
                   rotmay(euler_angles[1])*
                   rotmax(euler_angles[0]))
    return rot_mat

def ezxyrmx(euler_angles):
    rot_mat=rotmay(euler_angles[2]*
                   rotmax(euler_angles[1])*
                   rotmaz(euler_angles[0]))
    return rot_mat

def ezxzrmx(euler_angles):
    rot_mat=rotmaz(euler_angles[2]*
                   rotmax(euler_angles[1])*
                   rotmaz(euler_angles[0]))
    return rot_mat
